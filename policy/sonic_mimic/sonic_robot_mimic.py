...
class SONIC_ROBOT_Mimic(FSMState):
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        super().__init__()
        self.state_cmd = state_cmd
        self.policy_output = policy_output
        self.name = FSMStateName.SKILL_GAE
        self.name_str = "sonic_robot_mimic"
        self.counter_step = 0
        
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "SONIC_ROBOT_Mimic.yaml")
        
        # Check if config file exists
        if not os.path.exists(config_path):
            raise FileNotFoundError(
                f"Configuration file not found: {config_path}\n"
                f"Please ensure SONIC_ROBOT_Mimic.yaml exists in the config directory."
            )
    ...
    def get_robot_command(self):
        ...
        # (10, num_joints) 
        self.ref_joint_pos = self.motion_data["joint_pos"][step] # (10, num_joints)
        self.ref_joint_vel = self.motion_data["joint_vel"][step] # (10, num_joints)
        
        command = np.concatenate((self.ref_joint_pos, self.ref_joint_vel), axis=-1)
